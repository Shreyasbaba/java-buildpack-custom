...
if [ "$PRESERVE_JAVA_OPTS" != "true" ]; then
    ...
    if [ $CLIENT_VM = false ]; then
        ...
        if [ "x$NO_COMPRESSED_OOPS" = "x" ]; then
            "$JAVA" $JVM_OPTVERSION -server -XX:+UseCompressedOops -version  && PREPEND_JAVA_OPTS="$PREPEND_JAVA_OPTS -XX:+UseCompressedOops"
        fi
    fi
